...
class Decision(object):
	...
	def __init__(self, id,
			chamber=None, number=None, formation=None, publication=None,
			ecli=None, decision_date=None, solution=None, texts=None,
			content=None):
		self.kind = "decision"
		self.id = id
	...
	def to_json(self):
		return json.dumps(self.to_dict(), indent=2, ensure_ascii=False,
			default=self.custom_to_json)

	@classmethod
	def from_dict(cls, d:dict):
		return cls(id=d.get("id"), chamber=d.get("chamber"), formation=d.get("formation"),
			publication=d.get("publication"), ecli=d.get("ecli"), number=d.get("number"),
			decision_date=d.get("decision_date"), solution=d.get("solution"),
			texts=d.get("texts"), content=d.get("content"))
	...
	def __get_publication(data:str):
		...
		if m:
			return m.group('publication').replace('\n', '').replace('  ', '').replace('-', ' - ')
		return None
